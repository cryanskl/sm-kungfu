...
export interface Encounter {
  id: string;
  category: string;
  narrative: (heroName: string, heroName2?: string) => string;
  effects: {
    hp?: number;
    reputation?: number;
    hot?: number;
...
const ADVENTURES: Encounter[] = [
  ...
  },
  {
    ...
    factionAffinity: ['魔教'],
    personalityAffinity: ['aggressive'],
    tier: 'personal' as const,
    weight: 1,
    minRound: 3,
  },
...
// ============================================================
// 交汇事件 (Intersection) — 两个英雄同地点触发
// ============================================================

const INTERSECTION_ENCOUNTERS: Encounter[] = [
  {
    id: 'int_001',
    category: '交汇',
    narrative: (h1, h2) => `${h1}与${h2}在悬崖边不期而遇，二人互相试探了三招，各自暗叹对方功力深厚。`,
    effects: { reputation: 8, hot: 10 },
    tier: 'intersection' as const,
    locationTag: '悬崖',
    weight: 6,
  },
  {
    id: 'int_002',
    category: '交汇',
    narrative: (h1, h2) => `${h1}和${h2}同时发现了一坛地下埋藏的好酒，两人相视一笑，席地对饮。`,
    effects: { hp: 5, hot: 8 },
    tier: 'intersection' as const,
    locationTag: '酒馆',
    weight: 7,
  },
  {
    id: 'int_003',
    category: '交汇',
    narrative: (h1, h2) => `${h1}被毒蛇咬伤，${h2}恰好路过，用内力为其逼毒——二人由此结下善缘。`,
    effects: { hp: 8, reputation: 10, morality: 5 },
    tier: 'intersection' as const,
    locationTag: '山路',
    weight: 5,
  },
  {
    id: 'int_004',
    category: '交汇',
    narrative: (h1, h2) => `${h1}与${h2}在密林中追逐同一只锦鸡，撞在一起，大笑之余互换了一招独门武技。`,
    effects: { reputation: 8, hot: 12 },
    tier: 'intersection' as const,
    locationTag: '密林',
    weight: 6,
  },
  {
    id: 'int_005',
    category: '交汇',
    narrative: (h1, h2) => `暴雨之夜，${h1}和${h2}在同一座破庙避雨，彻夜长谈江湖往事。`,
    effects: { reputation: 5, credit: 5, hot: 5 },
    tier: 'intersection' as const,
    locationTag: '破庙',
    weight: 8,
  },
  {
    id: 'int_006',
    category: '交汇',
    narrative: (h1, h2) => `${h1}与${h2}同时闯入一处藏宝洞，在机关阵前被迫合作，历经险阻终于脱身。`,
    effects: { hp: -5, reputation: 12, hot: 10 },
    tier: 'intersection' as const,
    locationTag: '秘境',
    weight: 4,
    minRound: 2,
  },
  {
    id: 'int_007',
    category: '交汇',
    narrative: (h1, h2) => `${h1}在桥头与${h2}狭路相逢，二人各不相让，比试了一番轻功才分出谁先过桥。`,
    effects: { reputation: 8, hot: 8 },
    tier: 'intersection' as const,
    locationTag: '桥头',
    weight: 7,
  },
  {
    id: 'int_008',
    category: '交汇',
    narrative: (h1, h2) => `${h1}正在修炼掌法时被${h2}偷看，${h2}忍不住出言指点了一处破绽——二人相谈甚欢。`,
    effects: { reputation: 10, hot: 5 },
    tier: 'intersection' as const,
    locationTag: '练武场',
    weight: 6,
  },
  {
    id: 'int_009',
    category: '交汇',
    narrative: (h1, h2) => `${h1}和${h2}在茶楼各坐一侧，同时听到有人密谋刺杀武林盟主——两人目光相对，默契点头。`,
    effects: { reputation: 8, hot: 12, credit: 5 },
    tier: 'intersection' as const,
    locationTag: '茶楼',
    weight: 5,
    minRound: 3,
  },
  {
    id: 'int_010',
    category: '交汇',
    narrative: (h1, h2) => `${h1}与${h2}被困在山洪中，合力搬开巨石才得以逃生。患难见真情。`,
    effects: { hp: -8, reputation: 15, morality: 5 },
    tier: 'intersection' as const,
    locationTag: '山涧',
    weight: 4,
    minRound: 2,
  },
  {
    id: 'int_011',
    category: '交汇',
    narrative: (h1, h2) => `${h1}和${h2}在赌坊中对赌三局，你来我往，旁观者看得津津有味。`,
    effects: { hot: 15, reputation: 5 },
    tier: 'intersection' as const,
    locationTag: '赌坊',
    weight: 6,
  },
  {
    id: 'int_012',
    category: '交汇',
    narrative: (h1, h2) => `${h1}无意间撞见${h2}在月下独自练剑，剑光如水，看得入了神。两人以剑会友，收获良多。`,
    effects: { reputation: 10, hot: 8 },
    tier: 'intersection' as const,
    locationTag: '月下',
    weight: 5,
...
export const ALL_ENCOUNTERS: Encounter[] = [...ADVENTURES, ...INTERSECTION_ENCOUNTERS];
...
export function rollEncounters(
  ...
  for (let i = 0; i < actualCount; i++) {
    ...
    results.push({
      heroName: shuffledHeroes[i % shuffledHeroes.length],
      encounter,
    });
  }
...
/**
 * 按门派/性格亲和度为每个英雄分配独特奇遇
 *
 * 算法：
 * 1. 按回合过滤候选池
 * 2. 对每个英雄：优先从 factionAffinity/personalityAffinity 匹配的子池中抽取
 * 3. 已分配的奇遇从池中移除（同一局不重复）
 * 4. 每对英雄有概率触发交汇事件
 * 5. 返回结果中每个英雄获得 countPerHero 个独特奇遇
 */
export function rollPersonalEncounters(
  round: number,
  heroes: HeroInfo[],
  countPerHero: number,
): { heroName: string; heroId: string; encounter: Encounter; intersectWith?: string }[] {
  if (heroes.length === 0 || countPerHero <= 0) return [];
  ...
  const results: { heroName: string; heroId: string; encounter: Encounter; intersectWith?: string }[] = [];
  const usedEncounterIds = new Set<string>();

  // 按回合过滤交汇事件
  const eligibleIntersections = INTERSECTION_ENCOUNTERS.filter((e) => {
    if (e.minRound !== undefined && round < e.minRound) return false;
    if (e.maxRound !== undefined && round > e.maxRound) return false;
    return true;
  });
  ...
  const intersectionPairs = Math.min(Math.floor(heroes.length / 4), 2); // 最多 2 对交汇

  for (let p = 0; p < intersectionPairs && eligibleIntersections.length > 0; p++) {
    const h1 = shuffledHeroes[p * 2];
    const h2 = shuffledHeroes[p * 2 + 1];
    if (!h1 || !h2) break;

    const available = eligibleIntersections.filter(e => !usedEncounterIds.has(e.id));
    if (available.length === 0) break;

    const encounter = weightedPick(available);
    usedEncounterIds.add(encounter.id);
    ...
    results.push({
      ...
      heroId: h1.heroId,
      encounter,
      intersectWith: h2.heroName,
    });
    results.push({
      heroName: h2.heroName,
      heroId: h2.heroId,
      encounter,
      intersectWith: h1.heroName,
    });
  }

  // --- 个人奇遇：按亲和度分配 ---
  for (const hero of heroes) {
    // 跳过已经获得交汇事件的英雄的名额（交汇算 1 个名额）
    const existingCount = results.filter(r => r.heroId === hero.heroId).length;
    const remaining = countPerHero - existingCount;
    if (remaining <= 0) continue;

    for (let i = 0; i < remaining; i++) {
      let pool = eligible.filter(e => !usedEncounterIds.has(e.id));
      if (pool.length === 0) {
        // 池耗尽时重置（极端情况）
        usedEncounterIds.clear();
        pool = [...eligible];
